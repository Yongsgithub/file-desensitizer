...
## 🖥️ 本地离线 GUI（无需 AI / 无需联网）

**完全本地运行，数据绝不上云。** 适合没有 AI IDE 或需要离线处理的场景。

### Windows 用户

双击 `启动脱敏工具.bat` 即可启动图形界面。
...
|------|------|
| 📂 拖放上传 | 直接拖文件到窗口，或点击选择 |
| 🔒 一键脱敏 | 自动识别姓名/手机号/身份证/住址等 |
| 👁️ 结果预览 | 实时显示每条脱敏记录 |
| 🎨 遮挡方式 | 黑色块 / 模糊 / 像素化 可选 |
| 📁 输出目录 | 自定义脱敏文件保存位置 |
| ✅ 环境检查 | 启动时自动检测 Tesseract/LibreOffice |

### 命令行启动

```bash
# pip 安装后
file-desensitizer-gui
